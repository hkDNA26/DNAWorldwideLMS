    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="flex h-10 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg border-2 border-dashed border-slate-300 text-sm text-slate-500 hover:border-brand hover:text-brand transition-colors"
        >
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5">
      <div className="flex items-start justify-between mb-3">
        <div>
          <span className="text-xs font-medium text-brand">Q{index} · {typeLabel}</span>
        </div>
      ...
      <textarea
        className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent mb-4"
        rows={2}
      ...
      {question.type === "SHORT_ANSWER" ? (
        <div className="p-3 bg-slate-50 rounded-lg text-sm text-slate-500 italic">
          Staff will type a free-form answer (not auto-graded as correct/incorrect).
        </div>
      ) : (
        <div className="space-y-2">
          ...
          {question.answerOptions.map((option) => (
            <div key={option.id} className="flex items-center gap-2">
              ...
              />
              <input
                className="flex-1 border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-brand focus:border-transparent"
                value={option.text}
          ...
          {question.type === "MULTIPLE_CHOICE" && (
            <button
              onClick={addOption}
              className="flex items-center gap-1 text-xs text-brand hover:text-brand-dark mt-1"
            >
